<template>
	<PageWrapper>
		<SearchForm ref="searchForm" :formItems="formItems" @search="table.refresh()"></SearchForm>

		<STable ref="table" rowKey="id" :columns="columns" :data="reqData" :showPagination="true"
			:scroll="{ y: 'calc(100vh - 450px)' }">
			<template #toolbar>
				<a-button v-has="'Btn.CloudPlay.Add'" type="primary" @click="() => add.show()">
					添加
				</a-button>
			</template>
			<template #bodyCell="{ column, row }">
				<template v-if="column.dataIndex === 'action'">
					<a v-has="'Btn.CloudPlay.UpdateParams'" @click="() => editParams.show(row)">
						修改参数
					</a>
				</template>
			</template>
		...
		<EditParams ref="editParams" :cloudPlaform="cloudPlaform" @success="table.refresh()" />
	</PageWrapper>
</template>
