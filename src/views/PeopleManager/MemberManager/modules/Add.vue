	...
	<a-modal title="添加" :open="open" @ok="submit" @cancel="open = false">
		<a-form ref="formRef" :model="formState" :rules="rules" v-bind="layout">
			<a-form-item label="头像" name="img">
				<UploadImage v-model:value.trim="formState.img" placeholder="请输入"></UploadImage>
			</a-form-item>
			<a-form-item label="账户" name="zhanghu">
				<a-input v-model:value.trim="formState.zhanghu" placeholder="请输入手机号" />
			</a-form-item>
			<a-form-item label="密码" name="mima">
				<a-input-password v-model:value.trim="formState.mima" placeholder="请输入" />
			</a-form-item>
			<a-form-item label="名称" name="mingcheng">
				<a-input v-model:value.trim="formState.mingcheng" placeholder="请输入" />
			</a-form-item>
			<a-form-item label="会员等级" name="huiyuandengji">
				<a-input-number :min="0" v-model:value="formState.huiyuandengji" placeholder="请选择" style="width: 100%" />
			</a-form-item>
			<a-form-item label="启禁用" name="qijinyong">
				<a-radio-group v-model:value="formState.qijinyong">
					<a-radio-button v-for="item in qijinyong" :key="item.value" :value="item.value">
						{{ item.label }}
					</a-radio-button>
				</a-radio-group>
			</a-form-item>
			<a-form-item label="登录IP" name="dengluip">
				<a-textarea v-model:value.trim="formState.dengluip" placeholder="请输入"></a-textarea>
			</a-form-item>
		</a-form>
	</a-modal>
</template>
<script lang="ts" setup>
	...
	const rules = {
		...
		zhanghu: [{ required: true, pattern: phone, message: '账户不符合规范' }],
		mima: [
			{
				required: true,
				min: 6,
