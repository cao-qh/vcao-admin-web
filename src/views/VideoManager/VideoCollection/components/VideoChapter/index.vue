<template>
	<div>
		...
		<SearchForm :formItems="formItems" @search="table.refresh()" @reset="handleReset"></SearchForm>

		<STable ref="table" rowKey="id" :columns="columns" :data="reqData" :showPagination="true"
			...
			<template #bodyCell="{ column, row }">
				...
				<template v-if="column.dataIndex === 'suoluetu'">
					<a-image :width="50" :src="baseUrl + row.suoluetu + `?time=${dayjs().format('x')}`" />
				</template>
				<template v-if="column.dataIndex === 'shangxiajia'">
					<a-popconfirm title="确定要修改吗？" ok-text="是" cancel-text="否" @confirm="handleUpDown(row)">
				...
				<template v-if="column.dataIndex === 'action'">
					...
					<span>
						...
						<a-popconfirm title="确定删除吗？" ok-text="是" cancel-text="否" @confirm="handleDelete(row)">
							<a>删除</a>
						</a-popconfirm>
					</span>
				</template>
