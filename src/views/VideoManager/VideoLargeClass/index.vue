    ...
    >
      ...
      <template #bodyCell="{ column, row }">
        <template v-if="column.dataIndex === 'qijinyong'">
          <a-popconfirm
            v-if="userStore.hasPermission('Swh.VideoLargeClass.Enable')"
            title="确定要修改吗？"
            ok-text="是"
            cancel-text="否"
        ...
        <template v-if="column.dataIndex === 'action'">
          <a v-has="'Btn.VideoLargeClass.Update'" @click="() => edit.show(row)">
            修改
          </a>
        </template>
      </template>
...
import { reqSearch, reqEnable } from '@/api/VideoManager/VideoLargeClass'
