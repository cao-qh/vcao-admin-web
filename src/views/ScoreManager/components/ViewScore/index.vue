	...
	<div>
		<template v-if="data">
          ...
          jifenleixing.find((item) => item.value === data.jifenleixing)?.label
			...
			<a-space>
				<a-button v-has="'Btn.ViewScore.Update'" type="primary" @click="() => edit.show(data)">
					修改
				</a-button>
				<a-button v-has="'Btn.ViewScore.Delete'" type="primary" @click="handleDelete">
					删除
				</a-button>
			</a-space>
		</template>
		...
		<div v-else>
			<a-empty>
				...
				<a-button v-has="'Btn.ViewScore.Add'" type="primary" @click="add.show()">
					现在添加
				</a-button>
			</a-empty>
		</div>
